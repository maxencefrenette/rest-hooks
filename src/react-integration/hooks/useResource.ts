...
function useManyResources<A extends ResourceArgs<any, any, any>[]>(
  ...
  // only recompute final promise if params or resource change
  let promiseDeps: unknown[] = resourceList.map(
    ([selectShape, params]) => params && selectShape.getUrl(params),
  );
  promiseDeps = promiseDeps.concat(resources);
